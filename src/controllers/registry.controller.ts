...
export async function updateRadRequest(req: Request, res: Response, next: NextFunction) {
  ...
  if (status === 'dispensed') {
    ...
    try {
      ...
      // ถ้าไม่มี med_sid ให้ auto-pick คลังที่มีสต็อกมากสุดสำหรับ med_id นี้
      let med_sid = rad.med_sid ? Number(rad.med_sid) : null;
      if (!med_sid) {
        const { rows: swPick } = await client.query(
          `SELECT med_sid FROM ${SCHEMA}.med_subwarehouse
           WHERE med_id = $1 ORDER BY med_quantity DESC LIMIT 1`, [rad.med_id]
        );
        if (!swPick.length) throw new AppError('ไม่พบยาใน subwarehouse', 404);
        med_sid = Number(swPick[0].med_sid);
      }
      ...
      await client.query(
        ...
        [med_sid, rad.med_id, dispenseQty, balBefore, balAfter,
      ...
      const { rows } = await client.query(
        `UPDATE ${SCHEMA}.rad_registry
         SET status         = 'dispensed',
             ...
             updated_at     = NOW()
         WHERE rad_id = $4 RETURNING *`,
        [med_sid, approved_by ?? null, note, rad_id]
      );
...
export async function getMedProblems(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (search) {
      w += ` AND (mt.med_name ILIKE $${p} OR mt.med_generic_name ILIKE $${p} OR mt.med_thai_name ILIKE $${p} OR mp.description ILIKE $${p} OR CONCAT(pa.first_name,' ',pa.last_name) ILIKE $${p} OR pa.hn_number ILIKE $${p} OR pa.national_id ILIKE $${p})`;
      params.push(`%${search}%`); p++;
    }
