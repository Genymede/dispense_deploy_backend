...
export async function getMedProblems(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (search) {
      w += ` AND (mt.med_name ILIKE $${p} OR mt.med_generic_name ILIKE $${p} OR mt.med_thai_name ILIKE $${p} OR CONCAT(pa.first_name,' ',pa.last_name) ILIKE $${p} OR pa.hn_number ILIKE $${p} OR pa.national_id ILIKE $${p})`;
      params.push(`%${search}%`); p++;
    }
